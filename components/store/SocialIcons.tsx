...
export default function SocialIcons({ links, size = 20, className = "" }: SocialIconsProps) {
  const entries = (Object.keys(links) as Array<keyof SocialLinks>).filter((k) => links[k]);
    ...
    <div className={`flex items-center gap-5 ${className}`}>
      {entries.map((key) => {
        ...
        const href = key === "whatsapp"
          ? `https://wa.me/${links[key].replace(/\D/g, "")}`
          : links[key];
        return (
          <Link
            key={key}
